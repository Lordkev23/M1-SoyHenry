...
const calcularImpuestos = (factura, i = 0, acc = 0) => {
  // Tu código aquí:
  if(typeof(factura) === 'object' && !Array.isArray(factura)) return 'Error';
  if(factura.length === 0) return false;
  if(i === factura.length-1) return factura[factura.length-1];
  acc = factura[i] + calcularImpuestos(factura, i+1, acc);
