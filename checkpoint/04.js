...
function construccionCasas(bolsas) {
  // Tu código aquí:
  return function(casasRequeridas){
    let casasAConstruir = bolsas/10;
    if(casasRequeridas <= 0){
      return 'Por favor ingresar cuantas casas quieres construir';
    }
    if(casasRequeridas === casasAConstruir) return true;
    if(casasAConstruir < 1){
      return 'No se puede construir casas con esa cantidad de bolsas';
    }
    if(casasAConstruir > 0){
      return 'Solo puedes construir esta cantidad de casas: ' + Math.floor(casasAConstruir);
    }
  }
}
