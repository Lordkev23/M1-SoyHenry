...
function tipologiasSeparadas(arr) {
  ...
  for (let j = 0; j < nuevoArray.length; j++) {
	  nuevoStack.push(nuevoArray[j]);
  } 
  if(nuevoStack.size() === 0) return 'Tipologias inexistentes';
  if(numerosRepetidos === 0) return 'No hay tipologias repetidas';
	return nuevoStack;
}
